...
import {writeJSONData, writeStringData, svnCommit, writeByteArray} from "./writeJSONData";
...
class XLSXDecoder {
    ...
    constructor(gcfg: GlobalCfg, file: File, cPath: string, sPath: string, idx: number, cb: { (file: File, error: boolean) }) {
        ...
        for (let sheetName of sheetNames) {
            ...
            if (SHEET_TOJAT.indexOf(sheetName.toLowerCase()) > -1) {
                let jatPath = writeByteArray(sheetName, gcfg.serverPath, adatas, '.jat');
                if (jatPath) {
    ...
    \treturn ${getNameNoSymbol(define.name)};
            ...
            ${genManualAreaCode("$decode", cdict)}
                ...
                // 生成服务端代码
                let sdict = getManualCodeInfo(path.join(sPath, sfilePackage || "", fname + "TemplateGen.java"));
                if (sPros) {
                    let imps = "";
...
`
        ...
        svnCommit(fname, file.path, toXmlString(sDatas), '.xml');
 ...
 * @param {any[]} sdatas
 */
function writeDesignData(datas: any, flag: number, fname: string, gcfg: any) {
    // 导出客户端数据
    if (flag == 1) {
        let cpath = writeJSONData(fname, gcfg.clientPath, datas);
        if (cpath) {
    ...
    // 导出服务端数据
    if (flag == 2) {
        let spath = writeStringData(fname, gcfg.serverPath, toXmlString(datas), ".xml");
        if (spath) {
