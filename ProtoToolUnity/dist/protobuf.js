    ...
    ProtoBuf.DotProto = (function(ProtoBuf, Lang) {
        ...
        TokenizerPrototype.next = function() {
            ...
            do {
                ...
                this.backComment = "";
                // 注释
                var comment, commentStart;
                // Strip comments
                if (this.source.charAt(this.index) === '/') {
                    ++this.index;
                    if (this.source.charAt(this.index) === '/') { // Line
                        // 获取comment
                        commentStart = this.index + 1;
                        while (this.source.charAt(++this.index) !== '\n')
                            if (this.index == this.source.length)
                                return null;
                            // 得到 //后的注释
                        comment = this.source.substring(commentStart, this.index);
                        comment = comment.trim();
                        if (comment) {
                            if (hasNewLine) { // 检查注释前面是否有\n
                                //之前有换行 作为消息前面的注释处理
            ...
            this.frontComment = comments.join("<br/>");
            if (this.index === this.source.length)
                return null;
            // Read the next token
            var end = this.index;
        ...
        ParserPrototype._parseMessage = function(parent, fld) {
            var commentVal = this.tn.frontComment;
            var isGroup = !!fld,
                token = this.tn.next();
        ...
        ParserPrototype._parseMessageField = function(msg, rule, type) {
            ...
            var tn = this.tn;
            var idx = tn.index;
            var next, commentStart, comment;
            while (Lang.WHITESPACE.test(next = tn.source.charAt(idx))) {
                if (next === '\n') {
                    ++tn.line;
                    break;
                }
                if (++idx === tn.source.length)
                    break;
            }

            if (tn.source.charAt(idx) === '/') {
                ++idx;
                if (tn.source.charAt(idx) === '/') { // Line
                    // 获取comment
                    commentStart = idx + 1;
                    while (tn.source.charAt(++idx) !== '\n')
                        if (idx == tn.source.length)
                            break;
                        // 得到 //后的注释
                    comment = tn.source.substring(commentStart, idx);
                    comment = comment.trim();
                    if (comment) {
                        if (fld.comment) {
                            fld.comment += "<br/>" + comment;
                        } else {
                            fld.comment = comment;
                        }
                    }
