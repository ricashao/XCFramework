...
local function WsSerializeMessage(msg_obj, global_seq)
    local output = ""
    local send_msg = msg_obj.MsgProto:SerializeToString()

    output = output .. string.pack(">i4", msg_obj.MsgID)
    --code
    output = output .. string.pack(">I4", 0)
    output = output .. send_msg
    output = string.pack(">I4", string.len(send_msg) + 12) .. output
...
NetUtil.WsSerializeMessage = WsSerializeMessage
